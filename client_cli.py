...
{RESET}      {WHITE}The NoSQL Engine | v1.2 | @MemaroX
"""
...
class Za3bolaCompleter:
    ...
    def complete(self, text, state):
        ...
        if "USE " in buffer:
            # Autocomplete tables (simulated for now as we can't sync perfectly yet)
...
def run_client(host='127.0.0.1', port=8090):
    ...
    try:
        ...
        print(f"{Fore.BLUE}[*] Connected to Za3bolaDB at {host}:{port}")
        ...
        while True:
            try:
                ...
                # Update completer if we switch tables or add new ones (basic heuristic)
        ...
        print(f"{Fore.RED}[!] Could not connect to the server. Is it running?")
...
if __name__ == "__main__":
    import json # Needed for pretty-printing
    run_client()
